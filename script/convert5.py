...
def extract_course_codes_per_page(pdf_path):
    """Returns a list where each index is the course code for that page (1-based)."""
    course_codes_per_page = []
    last_code = None
    with pdfplumber.open(pdf_path) as pdf:
        for page in pdf.pages:
            page_text = page.extract_text() or ""
            match = re.search(r"SCHEDULE\s*:\s*([A-Z0-9]+)", page_text)
            if match:
...
def convert_pdf_to_json(pdf_path, json_path):
    ...
    course_codes_per_page = extract_course_codes_per_page(pdf_path)
...
if __name__ == "__main__":
    if len(sys.argv) < 3:
        print("Usage: python3 src/convert5.py input.pdf output.json")
        sys.exit(1)
    pdf_path = sys.argv[1]
