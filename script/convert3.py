            ...
            if tables:
                for table_idx, table in enumerate(tables):
                    ...
                    for row in table[start_row:]:
                        ...
                        entry_tuple = tuple(minimal_entry.items())
                        if is_valid_entry(minimal_entry) and entry_tuple not in unique_entries:
...
if __name__ == "__main__":
    if len(sys.argv) < 3:
        print("Usage: python3 src/convert3.py input.pdf output.json")
        sys.exit(1)
    pdf_path = sys.argv[1]
